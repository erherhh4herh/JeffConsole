...
// Stores data for the several JeffConsole components
public class JeffConsoleData
{
    // Log Storage
    ...
    public JeffConsole mainEditorConsole;
    ...
    private static JeffConsoleData instance;

    private JeffConsoleData()
    {
        init();
    }

    // Singleton
    public static JeffConsoleData Instance
    {
        get
        {
            if (instance == null)
            {
                instance = new JeffConsoleData();
            }
            return instance;
        }
    }
    ...
    {
        // Initialize counter
        logCounter[(int)JeffLogType.NORMAL] = 0;
        logCounter[(int)JeffLogType.WARNING] = 0;
        logCounter[(int)JeffLogType.ERROR] = 0;
    ...
    {
        JeffLogType BPType = JeffLogType.NORMAL;
        switch (type)
        {
            case UnityEngine.LogType.Assert:
            case UnityEngine.LogType.Error:
            case UnityEngine.LogType.Exception:
                BPType = JeffLogType.ERROR;
                break;
            case UnityEngine.LogType.Warning:
                BPType = JeffLogType.WARNING;
                break;
            case UnityEngine.LogType.Log:
                BPType = JeffLogType.NORMAL;
                break;
        }

        //    JeffConsole.Log(logString, "Default Console", BPType);
    }
...
public struct LogMessage
{
    ...
    public JeffLogType type;
    public DateTime stamp;
    public int messageID;

    private static int IDCounter = 0;
    // Constructor
    public LogMessage(string log, string tag, JeffLogType type, string stack)
    {
        this.log = log;
        this.tag = tag;
        this.type = type;
        ...
        {
            if (trace.Contains("JeffConsole.Log") || trace.Contains("get_StackTrace()"))
                continue;
        ...
        if (toAssign < len) // copy into Jeffaller array
        {
...
public struct StackTraceEntry
{
    ...
    public bool jumpToPath()
    {
        ...
        {
            UnityEngine.Object script = AssetDatabase.LoadAssetAtPath(filePath, typeof(UnityEngine.Object));
            if (script != null)
            {
