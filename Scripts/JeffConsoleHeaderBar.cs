...
public class JeffConsoleHeaderBar
{
    ...
    public void drawHeaderBar()
    {
        ...
        // soft reset
        if (_data.searchFilter != JeffConsoleData.DEFAULT_SEARCH_STR && prevSearch == JeffConsoleData.DEFAULT_SEARCH_STR)
        {
            if (_data.searchFilter.Length < JeffConsoleData.DEFAULT_SEARCH_STR.Length) // Something was removed
                _data.searchFilter = "";
            else if (_data.searchFilter.Length > JeffConsoleData.DEFAULT_SEARCH_STR.Length) // Something was added
            {
                _data.searchFilter = _data.searchFilter.Trim().Replace(JeffConsoleData.DEFAULT_SEARCH_STR, "");
            }
        }

        // will search
        if (_data.searchFilter != JeffConsoleData.DEFAULT_SEARCH_STR && prevSearch != _data.searchFilter)
        {
            searchLogs();
        }
        ...
        if (_data.showLogs = GUILayout.Toggle(_data.showLogs, "N(" + _data.logCounter[(int)JeffLogType.NORMAL] + ")", "ToolbarButton", new GUILayoutOption[1] { GUILayout.Width(80) }))
            normalButton();

        if (_data.showWarnings = GUILayout.Toggle(_data.showWarnings, "W(" + _data.logCounter[(int)JeffLogType.WARNING] + ")", "ToolbarButton", new GUILayoutOption[1] { GUILayout.Width(80) }))
            warningButton();

        if (_data.showErrors = GUILayout.Toggle(_data.showErrors, "E(" + _data.logCounter[(int)JeffLogType.ERROR] + ")", "ToolbarButton", new GUILayoutOption[1] { GUILayout.Width(80) }))
            errorButton();
    ...
    void clearSearchButton()
    {
        ...
        _data.searchFilter = JeffConsoleData.DEFAULT_SEARCH_STR;
    }
