...
public class JeffConsoleTopSection
{
    JeffConsoleData _data;
    GUISkin _buttonSkin;
    ...
    public JeffConsoleTopSection()
    {
        // create skins for buttons
        _data = JeffConsoleData.Instance;
        _buttonSkin = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SKINS_DIR + "/JeffConsoleSkin.guiskin", typeof(GUISkin)) as GUISkin;
        _buttonSkin2 = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SKINS_DIR + "/JeffConsoleSkin2.guiskin", typeof(GUISkin)) as GUISkin;
        _selectedButtonSkin = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SKINS_DIR + "/JeffConsoleSkin3.guiskin", typeof(GUISkin)) as GUISkin;
        _logSkin = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SKINS_DIR + "/LogButton.guiskin", typeof(GUISkin)) as GUISkin;

        // create textures for icons
        _logTex = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SPRITES_DIR + "/log.png", typeof(Texture2D)) as Texture2D;
        _warningTex = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SPRITES_DIR + "/warning.png", typeof(Texture2D)) as Texture2D;
        _errorTex = AssetDatabase.LoadAssetAtPath(JeffConsole.ASSETS_PATH + JeffConsole.SPRITES_DIR + "/error.png", typeof(Texture2D)) as Texture2D;
    }
    ...
    public void drawTopSection(float width, float height)
    {
        ...
        if (_logTex == null)
        {
            Debug.Log("[JeffConsole] invalid initialization of assets");
            return;
        }
        */

        _labelHeight = 24;
        _labelWidth = (width * 0.8f + 3) - 24 * 2;
    ...
    void drawLogWindow()
    {
        ...
        if (!_data.canCollapse) // Draw normal messages
        {
            ...
            {
                logCount++;
                bool canDisplay = iJeffessageTypeAvailable(message.type) && iJeffessageTagAvailable(message.tag);
    ...
    void displayMessage(LogMessage message, GUISkin skin, bool isSelected)
    {
        ...
        // Timer
        selected |= GUILayout.Button(JeffConsoleData.getTimeStamp(message.stamp), skin.button, GUILayout.Width(_labelWidth * 0.15f), GUILayout.Height(_labelHeight));
    ...
    bool drawIconLabel(JeffLogType type, GUISkin skin)
    {
